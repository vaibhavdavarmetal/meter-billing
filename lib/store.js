...
export async function getPeriodReadings(period, slugs) {
  const out = {};
  for (const slug of slugs) out[slug] = await getReading(period, slug);
  return out;
}
...
export async function getPeriodBills(period, slugs) {
  const out = {};
  for (const slug of slugs) out[slug] = await getBill(period, slug);
  return out;
}
...
export async function getPeriodApprovals(period, slugs) {
  const out = {};
  for (const slug of slugs) out[slug] = await getApproval(period, slug);
  return out;
}
