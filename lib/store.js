...
// Fetch bills for one slug across many periods in a SINGLE batched call.
export async function getBillsForPeriods(periods, slug) {
  const vals = await kvGetMany(periods.map((p) => billKey(p, slug)));
  const out = {};
  periods.forEach((p, i) => { out[p] = vals[i]; });
  return out;
}
...
// ── Accounts locker (utility / service reference details; owner-only) ──
const ACCOUNTS_KEY = "accounts:v1";
export async function getAccounts() { return kvGet(ACCOUNTS_KEY); }
export async function saveAccounts(data) { return kvSet(ACCOUNTS_KEY, data); }
...
// ── Move-out settlement (one per tenant, keyed by slug) ──
const settlementKey = (slug) => `settlement:${slug}`;
export async function getSettlement(slug) { return kvGet(settlementKey(slug)); }
export async function saveSettlement(slug, data) { return kvSet(settlementKey(slug), data); }
